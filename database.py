...
class Database:
    ...
    def record_signal(self, signal: dict, timeframe: str = "M5", run_id: str | None = None) -> None:
        with self.connect() as conn:
            conn.execute(
                """
    ...
    def record_trade_attempt(
        ...
        with self.connect() as conn:
            conn.execute(
                """
                INSERT INTO trades (run_id, created_at, pair, timeframe, side, score,
                                    status, payload_json, broker_trade_id, note)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    run_id,
    ...
    def purge_old_data(self, retention_days: int = 90, vacuum: bool = False) -> dict[str, Any]:
        """Delete rows older than the rolling retention window.

        With retention_days=90, only rows outside the latest 90-day window
        are removed. Newer rows always remain intact.
        """
        ...
        with self.connect() as conn:
            cycle_deleted  = conn.execute(
                "DELETE FROM cycle_runs WHERE started_at < ?", (cutoff_iso,)
            ).rowcount
            signal_deleted = conn.execute(
                "DELETE FROM signals_log WHERE logged_at < ?", (cutoff_iso,)
            ).rowcount
            trade_deleted  = conn.execute(
                "DELETE FROM trades WHERE created_at < ?", (cutoff_iso,)
            ).rowcount
            conn.commit()
            if vacuum:
                conn.execute("VACUUM")
        ...
        self.upsert_state("last_retention_cleanup", {**summary, "ran_at": utc_now_iso()})
        return summary

    def query_blocked_cycles(self, date_utc_prefix: str) -> dict[str, int]:
        """Count SKIPPED cycle runs for a calendar day by block stage.

        date_utc_prefix — e.g. '2026-05-16' (matches started_at LIKE '2026-05-16%')
        ...
        for row in rows:
            try:
                ...
                elif stage in ("signal_validation", "cascade_fail") or reason == "signal_blocked":
                    counts["signal_blocked"] += 1
                elif stage not in (
                    "daily_caps", "market_guard", "chaos_filter",
                    "gap_filter", "session_check", "cooldown_guard",
                    "circuit_breaker", "daily_goal_reached",
                ):
                    counts["other"] += 1
            except Exception:
                pass
